...
if ($dashboardurl == 'Course') {
    if (is_siteadmin($USER->id) || (new ls)->is_manager($USER->id, $_SESSION['ls_contextlevel'], $_SESSION['role'])) {
        $dashboardcourse = $DB->get_field_sql("SELECT id FROM {course} WHERE visible = 1 AND id <> " . SITEID . ' ORDER BY id ASC LIMIT 0, 1');
        ...
        $dashboardcourse = (new querylib)->get_rolecourses($USER->id, $role, $contextlevel, SITEID, '', ' LIMIT 0, 1', false, false, '', true);
