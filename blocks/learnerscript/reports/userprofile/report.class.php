...
class report_userprofile extends reportbase {
    ...
    function select() {
        ...
        $this->sql = " SELECT DISTINCT user.id , CONCAT(user.firstname,' ',user.lastname) AS fullname ";
        parent::select();
    }
    ...
    function where() {
        ...
        $this->sql .=" WHERE user.confirmed = 1 AND user.deleted = 0 AND user.id IN ($userid)  
                        AND ra.timemodified BETWEEN $this->ls_startdate AND $this->ls_enddate";
    ...
    public function column_queries($column, $userid){
       ...
       $learnercoursesql  = (new querylib)->get_learners('u.id','');
        switch ($column) {
            ...
            case 'inprogress':
                $identy = "ue.userid";
                $query = "SELECT (COUNT(DISTINCT c.id) - COUNT(DISTINCT cc.course)) AS inprogress 
                          FROM {user_enrolments} ue   
                          JOIN {enrol} e ON ue.enrolid = e.id AND e.status = 0
                          JOIN {role_assignments} ra ON ra.userid = ue.userid
                          JOIN {role} r ON r.id = ra.roleid AND r.shortname = 'student'
