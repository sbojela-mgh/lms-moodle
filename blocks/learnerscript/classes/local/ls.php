...
class ls {
	...
	public function learnerscript_sections_dynamic_columns($columns, $config, $basicparams){
		...
		foreach ($columns as $colvalue) {
	        ...
	        if ($colvalue->pluginname == "cohortusercolumns") {
	            ...
	            $pluginname = $colvalue->pluginname;
	            $plgname = 'plugin_cohortusercolumns';
	            $dynclass = new $plgname($config, $colvalue);
	            $flag = 1;
	        }
	    }
    ...
    public function userscormtimespent() {
    	...
    	foreach ($scormdetails as $scormdetail) {
	        ...
	        $insertdata->timespent = ROUND($this->timetoseconds($scormdetail->time), 0);
	        ...
	        $insertdata1->timespent = ROUND($this->timetoseconds($scormdetail->time), 0);
	        ...
	        if (!empty($records1)) {
	        	foreach ($records1 as $record1) {
		            ...
		            $insertdata1->timespent += ROUND($record1->timespent,0);
		    ...
		    if ($insertdata->instanceid != 0) {
		        if (!empty($records)) {
		        	foreach ($records as $record) {
			            ...
			            $insertdata->timespent += ROUND($record->timespent,0);
    ...
    public function userquiztimespent() {
    	...
    	foreach ($quizdetails as $quizdetail) {
	        ...
	        $insertdata->timespent = ROUND($quizdetail->time, 0);
	        $insertdata->activityid = $coursemoduleid;
	        $insertdata->timecreated = time();
	        $insertdata->timemodified = 0;
	        $insertdata1 = new stdClass();
	        ...
	        $insertdata1->courseid = $courseid;
	        $insertdata1->timespent = ROUND($quizdetail->time,0);
	        $insertdata1->timecreated = time();
	        $insertdata1->timemodified = 0;
	        $records1 = $DB->get_records('block_ls_coursetimestats',
	                    array('userid' => $insertdata1->userid,
 ...
 public function userbigbluebuttonbnspent() {
        ...
        $bigbluebuttonbnleftdetails = $DB->get_records_sql("SELECT lsl.timecreated, lsl.userid,lsl.courseid, lsl.contextinstanceid,lsl.id FROM mdl_logstore_standard_log lsl JOIN mdl_user u ON u.id = lsl.userid JOIN mdl_course c ON c.id = lsl.courseid WHERE lsl.action = 'left' AND lsl.component = 'mod_bigbluebuttonbn' AND u.confirmed = 1 AND u.deleted = 0 AND lsl.timecreated > ".$bbcrontime." AND lsl.userid > 2 ORDER BY lsl.id DESC");
        if (empty($bigbluebuttonbnleftdetails)) {
            return true;
        }
        foreach ($bigbluebuttonbnleftdetails as $bigbluebuttonbnleftdetail) {
            $bigbuttonid = $DB->get_field('bigbluebuttonbn', 'id', array('course' => $bigbluebuttonbnleftdetail->courseid));
            $bigbluebuttonbnjoindetails = $DB->get_field_sql("SELECT lsl.timecreated
             FROM mdl_logstore_standard_log lsl JOIN mdl_user u ON u.id = lsl.userid JOIN mdl_course c ON c.id = lsl.courseid WHERE lsl.action = 'joined' AND lsl.component = 'mod_bigbluebuttonbn' AND u.confirmed = 1 AND u.deleted = 0 AND lsl.contextinstanceid =".$bigbluebuttonbnleftdetail->contextinstanceid." AND lsl.userid =". $bigbluebuttonbnleftdetail->userid." AND lsl.courseid = ".$bigbluebuttonbnleftdetail->courseid." AND lsl.timecreated < ".$bigbluebuttonbnleftdetail->timecreated." AND lsl.timecreated > ".$bbcrontime." AND lsl.userid > 2 ORDER BY lsl.id DESC LIMIT 0,1 ");

            $bigbluebuttonbnrejoindetails = $DB->get_field_sql("SELECT MAX(lsl.timecreated)
             FROM mdl_logstore_standard_log lsl JOIN mdl_user u ON u.id = lsl.userid JOIN mdl_course c ON c.id = lsl.courseid WHERE lsl.action = 'joined' AND lsl.component = 'mod_bigbluebuttonbn' AND u.confirmed = 1 AND u.deleted = 0 AND lsl.contextinstanceid =".$bigbluebuttonbnleftdetail->contextinstanceid." AND lsl.userid =". $bigbluebuttonbnleftdetail->userid." AND lsl.courseid = ".$bigbluebuttonbnleftdetail->courseid." AND lsl.id > ".$bigbluebuttonbnleftdetail->id." AND lsl.timecreated > ".$bbcrontime." AND lsl.userid > 2 ORDER BY lsl.id "); 
                if (!empty($bigbluebuttonbnrejoindetails)) {
                      $bigbuttonclosetime = $DB->get_field('bigbluebuttonbn', 'closingtime', array('id' => $bigbuttonid));
            ...
            // if(!empty($bigbuttonclosetime)){
            //         if($bigbuttonclosetime > $bigbluebuttonbnrejoindetails){
            //           $insertdata->timespent +=   $bigbuttonclosetime - $bigbluebuttonbnrejoindetails;
            //         }   
            //  } 
            ...
            // if(!empty($bigbuttonclosetime)){
            //         if($bigbuttonclosetime > $bigbluebuttonbnrejoindetails){
            //           $insertdata1->timespent +=   $bigbuttonclosetime - $bigbluebuttonbnrejoindetails;
            //         }   
            // }        
