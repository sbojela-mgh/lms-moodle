    ...
    public function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0,$reporttype=null) {
        ...
        switch($data->column) { 
            ...
            case 'inactivestudents':  
                ...
                $enrolledusers = $DB->get_record_sql("SELECT COUNT(DISTINCT ue.userid) AS enrolled 
                                    ...
                                    where 1 = 1 AND ra.userid IN ($learnersql) AND ct.instanceid = $row->courseid"); 
                if (!empty($enrolledusers->enrolled)) {
                    $row->{$data->column} = $enrolledusers->enrolled - $row->activestudents; 
                }
            break; 
            case 'learner': 
