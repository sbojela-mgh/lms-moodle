...
require_once($CFG->dirroot.'/enrol/applicationenrolment/classes/forms/text_question.php');

use enrol_applicationenrolment\text_question;
...
$PAGE->set_url(new moodle_url('/enrol/applicationenrolment/addtextquestion.php'), []);

$pagetitle = get_string('pageheading_textquestion', 'enrol_applicationenrolment');
if ($questionid != 0) {
...
// Display the form
$form = new text_question(new moodle_url('/enrol/applicationenrolment/addtextquestion.php'), []);
...
if ($form->is_cancelled()) {
    ...
    $obj_question->question_content = json_encode([
                                        ...
                                        'inbox_text_size' => $formadata->inbox_text_size,
                                        ]);

    $obj_question->categoryid       = $formadata->category;
    $obj_question->required         = $formadata->required == 1 ? 1 : 0;
    $obj_question->allow_attachments    = $formadata->allow_attachments == 1 ? 1 : 0;
    $obj_question->required_attachments = $formadata->required_attachments == 1 ? 1 : 0;
